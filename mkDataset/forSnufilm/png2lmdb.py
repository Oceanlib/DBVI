...
def png2LMDB(srcDir, lmdb_path):
    ...
    allKeys = []
    with open('Path/to/snufilm-test/snufilm_lmdb/test-easy.pkl', 'rb') as fs:
        samples = pickle.load(fs)
    for asample in samples:
        allKeys.extend(list(asample.values()))

    with open('Path/to/snufilm-test/snufilm_lmdb/test-extreme.pkl', 'rb') as fs:
        samples = pickle.load(fs)
    for asample in samples:
        allKeys.extend(list(asample.values()))

    with open('Path/to/snufilm-test/snufilm_lmdb/test-hard.pkl', 'rb') as fs:
        samples = pickle.load(fs)
    for asample in samples:
        allKeys.extend(list(asample.values()))

    with open('Path/to/snufilm-test/snufilm_lmdb/test-medium.pkl', 'rb') as fs:
        samples = pickle.load(fs)
    for asample in samples:
        allKeys.extend(list(asample.values()))
    allKeys = set(allKeys)
...
if __name__ == '__main__':
    srcDir = 'Path/to/snufilm-test'
    lmdb_path = 'Path/to/snufilm/snufilm_lmdb'
