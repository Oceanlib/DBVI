...
class Train(data.Dataset):
    def __init__(self, cfg: configMain):
        self.Sample = './datasets/GoPro/gopro_train_lmdb/sample.pkl'
        self.LMDB = './datasets/GoPro/gopro_train_lmdb/data.mdb'
...
class celebAValidation(data.Dataset):
    def __init__(self, cfg: configMain):
        super(celebAValidation, self).__init__()
        self.Sample = './datasets/GoPro/gopro_test_lmdb/sample.pkl'
        # self.Sample = './datasets/GoPro/gopro_test_lmdb/sample4x.pkl'  # for testing 4x interpolation
        self.LMDB = './datasets/GoPro/gopro_test_lmdb/data.mdb'
        self.numIter = cfg.numIter
        ...
        self.transforms = cvtransform.Compose([
            # cvtransform.CenterCrop(cfg.test.size),
            cvtransform.ToTensor()
        ])
        self.env = None
        self.txn = None
...
def creatValiLoader(cfg: configMain):
    ...
    loader = data.DataLoader(dataset=dataset, batch_size=1, shuffle=False,
                             num_workers=2, pin_memory=True,  # False if memory is not enough
...
if __name__ == "__main__":
    cfg = configMain()
    testSampler, testLoader = creatTrainLoader(cfg)
