...
class _FunctionSoftsplat(torch.autograd.Function):
    ...
    def forward(self, input, flow):
        ...
        # self.save_for_backward(input.clone(), flow.clone())
        self.save_for_backward(input, flow.clone())
        ...
        if input.is_cuda == True:
            if gradInput is not None:
                ...
                cupy_launch('kernel_Softsplat_updateGradInput', cupy_kernel('kernel_Softsplat_updateGradInput', {
                    ...
                    grid=tuple([int((n + 512 - 1) / 512), 1, 1]),
                    block=tuple([512, 1, 1]),
                    args=[cupy.int32(n), input.data_ptr(), flow.data_ptr(), gradOutput.data_ptr(), gradInput.data_ptr(),
                          None],
                    stream=collections.namedtuple('Stream', 'ptr')(torch.cuda.current_stream().cuda_stream)
                )
            # end
            ...
            if gradFlow is not None:
                ...
                cupy_launch('kernel_Softsplat_updateGradFlow', cupy_kernel('kernel_Softsplat_updateGradFlow', {
                    ...
                    grid=tuple([int((n + 512 - 1) / 512), 1, 1]),
                    block=tuple([512, 1, 1]),
                    args=[cupy.int32(n), input.data_ptr(), flow.data_ptr(), gradOutput.data_ptr(), None,
                          gradFlow.data_ptr()],
                    stream=collections.namedtuple('Stream', 'ptr')(torch.cuda.current_stream().cuda_stream)
                )
        # end
